...
<%--        <dxe:ASPxComboBox ID="ddlCertificateNo" runat="server">
        ...
        <Columns>
            ...
            <%--<dxwgv:GridViewDataTextColumn FieldName="StoneCount" VisibleIndex="8" Caption="Stone Count">
            </dxwgv:GridViewDataTextColumn>--%>
        </Columns>
        </dxwgv:ASPxGridView>
        </div> 
        ...
        SelectCommand="select PC.CertificateNo, PC.ProductNo, PC.CategoryCode, PC.SubCategoryCode, PC.CreatedDate, PC.CreatedBy ,PC.Printed,PC.StoneCount from ( select a.CertificateNo, a.ProductNo, a.CategoryCode, a.SubCategoryCode, a.CreatedDate, a.CreatedBy ,a.Printed,case when (select count(1) as CountStone  from T_stone s where s.CertificateNo=a.CertificateNo) = 0 then 0 else (select count(1) as CountStone  from T_stone s where s.CertificateNo=a.CertificateNo) end as StoneCount from ( SELECT T_Certificate.CertificateNo, T_Certificate.ProductNo, T_ProductCategory.CategoryCode, T_ProductSubCategory.SubCategoryCode, T_Certificate.CreatedDate, T_Certificate.CreatedBy , Case when T_Certificate.PrintCount > 0 then 'Y' else 'N' end as Printed  FROM T_ProductSubCategory INNER JOIN T_Certificate INNER JOIN T_ProductSeries ON T_Certificate.ProductSeries = T_ProductSeries.id INNER JOIN T_ProductCategory ON T_ProductSeries.ProductCategory = T_ProductCategory.id ON T_ProductSubCategory.id = T_ProductSeries.ProductSubCategory where T_Certificate.Status = 1 ) as a ) as PC order by PC.CertificateNo desc">
    </asp:SqlDataSource>

        <!--original query before fixed crystal report-->
